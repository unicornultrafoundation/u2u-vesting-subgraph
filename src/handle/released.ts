...
import {Released as WrapperReleased} from "../../generated/VestingWrapper/VestingWrapper";
...
export function released(e: Released): void {
  ...
  userPool.completedPeriods = userPool.completedPeriods.plus(e.params.releasedPeriods);
...
export function wrapperReleased(e: WrapperReleased): void {
  log.info("New wrapper release with txHash: {}, block: {}", [e.transaction.hash.toHexString(), e.block.number.toString()])

  let vestingPoolID = e.params.pool.toHexString();
  let vestingPool = VestingPool.load(vestingPoolID);
  if (!vestingPool) {
    log.info("cannot release from invalid vesting pool {}", [vestingPoolID]);
    return;
